...
export async function GET(request: NextRequest) {
  try {
    ...
    const empresa = await prisma.empresa.findUnique({
      ...
      select: { nome: true, cnpj: true, endereco: true, cidade: true, uf: true, logo: true },
    });
    ...
    if (empresa?.logo) {
      const logoData = await imageToBase64(empresa.logo);
      if (logoData) {
        try {
          doc.addImage(
    ...
    const titleX = empresa?.logo ? marginLeft + 24 : marginLeft;

    doc.setFontSize(16);
    doc.setFont("helvetica", "bold");
    doc.text("Relatório de Recibos", titleX, yPos + 8);
